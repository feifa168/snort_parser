...
CREATE TABLE `alert` (
  ...
  `gid` int(11) DEFAULT NULL COMMENT '告警分类，主分类',
  `sid` int(11) DEFAULT NULL COMMENT '告警分类，次分类',
  `rid` int(11) DEFAULT NULL COMMENT '告警分类，次分类小版本号',
  `msg` varchar(4096) DEFAULT NULL COMMENT '告警内容',
  `priority` int(11) DEFAULT NULL COMMENT '日志中的priorty值',
  `proto` varchar(32) DEFAULT NULL COMMENT '协议类型',
  `sip` varchar(128) DEFAULT NULL COMMENT '告警源ip',
  `sport` int(11) DEFAULT NULL COMMENT '端口号',
  `isleft2right` tinyint(1) DEFAULT NULL COMMENT 'true为sip->dip,false为sip<-dip',
  `dip` varchar(128) DEFAULT NULL COMMENT '告警目的ip',
